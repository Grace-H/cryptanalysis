...
int build_freq_table(char *text, double *freq);
...
int main(int argc, char **argv){
  ...
  frequencies = malloc(sizeof(double *) * MAX_KEY_SIZE);
  int i;
  for(i = 0; i < MAX_KEY_SIZE; i++){
    frequencies[i] = malloc(sizeof(double) * 26);
  }
...
int build_freq_table(char *text, double *table){
